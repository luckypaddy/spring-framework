...
public class ViewResolutionRegistryTests {
	...
	public void noViewResolution() {
		...
		assertEquals(0, registry.getConfigurers().size());
	}
	...
	public void beanNameViewResolution() {
		...
		assertNotNull(registry.getConfigurers());
		assertEquals(0, registry.getConfigurers().size());
	}
	...
	public void tilesViewResolution() {
		registry.tiles(new TilesConfigurer()).checkRefresh(true).definition("def1").definition("def2");
		assertNotNull(registry.getViewResolvers());
		assertEquals(1, registry.getViewResolvers().size());
		assertEquals(TilesViewResolver.class, registry.getViewResolvers().get(0).getClass());

		assertNotNull(registry.getConfigurers());
		assertEquals(1, registry.getConfigurers().size());
		TilesConfigurer configurer = (TilesConfigurer)registry.getConfigurers().get(0);
	...
	public void velocityViewResolution() {
		registry.velocity(new VelocityConfigurer()).prefix("/").suffix(".vm").cache(true).resourceLoaderPath("testResourceLoaderPath");
		assertNotNull(registry.getViewResolvers());
		assertEquals(1, registry.getViewResolvers().size());
		...
		assertNotNull(registry.getConfigurers());
		assertEquals(1, registry.getConfigurers().size());
		assertEquals(VelocityConfigurer.class, registry.getConfigurers().get(0).getClass());
		VelocityConfigurer configurer = (VelocityConfigurer)registry.getConfigurers().get(0);
		DirectFieldAccessor configurerDirectFieldAccessor =  new DirectFieldAccessor(configurer);
	...
	public void defaultVelocityViewResolution() {
		registry.velocity(new VelocityConfigurer());
		assertNotNull(registry.getViewResolvers());
		assertEquals(1, registry.getViewResolvers().size());
		assertEquals(VelocityViewResolver.class, registry.getViewResolvers().get(0).getClass());
		VelocityViewResolver resolver = (VelocityViewResolver)registry.getViewResolvers().get(0);
	...
	public void freeMarkerViewResolution() {
		registry.freemarker(new FreeMarkerConfigurer()).prefix("/").suffix(".fmt").cache(false).templateLoaderPath("path1").templateLoaderPath("path2");
		assertNotNull(registry.getViewResolvers());
		assertEquals(1, registry.getViewResolvers().size());
		...
		assertNotNull(registry.getConfigurers());
		assertEquals(1, registry.getConfigurers().size());
		assertEquals(FreeMarkerConfigurer.class, registry.getConfigurers().get(0).getClass());
		FreeMarkerConfigurer configurer = (FreeMarkerConfigurer)registry.getConfigurers().get(0);
		DirectFieldAccessor configurerDirectFieldAccessor =  new DirectFieldAccessor(configurer);
	...
	public void defaultFreeMarkerViewResolution() {
		registry.freemarker(new FreeMarkerConfigurer());
		assertNotNull(registry.getViewResolvers());
		assertEquals(1, registry.getViewResolvers().size());
		assertEquals(FreeMarkerViewResolver.class, registry.getViewResolvers().get(0).getClass());
		FreeMarkerViewResolver resolver = (FreeMarkerViewResolver)registry.getViewResolvers().get(0);
